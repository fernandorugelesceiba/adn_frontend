...
@Component({
  selector: 'app-verificar-credenciales-cliente',
  templateUrl: './verificar-credenciales-cliente.component.html',
  styleUrls: ['./verificar-credenciales-cliente.component.css']
})
export class VerificarCredencialesClienteComponent implements OnInit {
  ...
  consultarPorTipoYNumeroDocumento() {
    ...
    this.clienteService.verificarClienteSegunNumeroYTipoDocumento(this.clienteFormulario.value).subscribe(res => {
      if (res.length > 0) {
        let clienteRes = res[0];
        this.cliente = new Cliente(clienteRes.id, clienteRes.nombre, clienteRes.apellido, clienteRes.tipoDocumento, clienteRes.numeroDocumento, clienteRes.fechaCreacion, clienteRes.idUsuarioCreacion);
